Apigility RESTful API
==============================

RPC and RESTful API according `https://api-tools.getlaminas.org/documentation/intro/getting-started`
...
```bash
# git clone https://github.com/marcelohweb/apigility.git
$ cd path/to/install
```
...
Navigate to `Authentication` tab on Apigility UI and click `New Adapter`.

Type adapter name, select the HTTP Basic type, provide a realm value and type `./users.htpasswd` to use the password file already created.

Once the adapter has been created, you must to assign this Adapter to Status API REST Service. Click on Status API (left side) and select the new adapter for authentication type combo.

A configuration is generated in config/autoload/local.php file. Edit this file to add the follow code:
...
For OAuth2 method, see `https://api-tools.getlaminas.org/documentation/auth/authentication-oauth2`

To interacting with Status RESTful API, use Postman client. Open Postman and import the Apigility-RESTful.postman_collection.json collection file.

This collection contains request for all API routes. Go to `Documentation` section on Apigility UI to see the API documentation. 

Authentication data is defined in the Authorization tab:
